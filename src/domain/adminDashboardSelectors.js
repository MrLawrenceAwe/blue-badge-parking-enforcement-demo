...
export function selectFilteredAdminBadges({ badges, sessions, scans, filters, verificationByBadge }) {
  return badges.filter((badge) => {
    const verification = verificationByBadge[badge.id];
    ...
    const matchesLocation =
      ...
      activityRecords.some((activityRecord) =>
        activityRecord.location?.toLowerCase().includes(filters.location.toLowerCase()),
      );
    const matchesDate =
      !filters.date ||
...
export function selectAdminDashboardData({
  ...
  const filteredBadges = selectFilteredAdminBadges({ badges, sessions, scans, filters, verificationByBadge });
  ...
  const filteredCases = cases.filter(
    (caseRecord) => filteredBadgeIds.has(caseRecord.badgeId) || !knownBadgeIds.has(caseRecord.badgeId),
  );

  return {
    filteredBadges,
    filteredActiveSessions: sessions.filter(
      (session) => isSessionActive(session) && filteredBadgeIds.has(session.badgeId),
    ),
