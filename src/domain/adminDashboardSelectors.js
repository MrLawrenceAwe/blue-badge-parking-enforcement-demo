...
export function buildAdminDashboard({ badges, sessions, scans, cases, filters, verificationByBadge, selectedBadgeId }) {
  ...
  return {
    ...
    reviewQueueCases: filteredCases.filter((caseRecord) => {
      ...
      return isCaseOpen(caseRecord) && (verification?.score >= 31 || ['Officer review', 'High priority', 'Evidence requested'].includes(caseRecord.status));
    }),
