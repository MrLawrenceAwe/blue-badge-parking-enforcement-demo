...
export function CaseRecordCard({
  ...
  return (
    <article className="case-card">
      ...
      <label>Status<select value={caseRecord.status} onChange={(event) => caseWorkflowActions.updateCase(caseRecord.id, { status: event.target.value })}>{caseStatuses.map((status) => <option key={status}>{status}</option>)}</select></label>
      <label>Assigned team<input value={caseRecord.assignedTeam} onChange={(event) => caseWorkflowActions.updateCase(caseRecord.id, { assignedTeam: event.target.value })} aria-label={`Assigned team for ${caseRecord.id}`} /></label>
      <div className="case-field-grid">
        <label>Due date<input type="date" value={caseRecord.dueDate ?? ''} onChange={(event) => caseWorkflowActions.updateCase(caseRecord.id, { dueDate: event.target.value })} aria-label={`Due date for ${caseRecord.id}`} /></label>
        <label>Closure reason<input value={caseRecord.closureReason ?? ''} onChange={(event) => caseWorkflowActions.updateCase(caseRecord.id, { closureReason: event.target.value })} aria-label={`Closure reason for ${caseRecord.id}`} /></label>
      </div>
      ...
      <button className="secondary-button" type="button" onClick={() => caseWorkflowActions.appendCaseNote(caseRecord.id)}><FileText aria-hidden="true" size={18} /> Add note</button>
