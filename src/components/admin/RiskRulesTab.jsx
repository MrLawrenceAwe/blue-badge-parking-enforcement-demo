...
export function RiskRulesTab({ riskRules, updateRiskRule, riskRuleNotice }) {
  return (
    <div className="app-panel risk-rules-panel full-span">
      <div className="app-panel-heading"><h2>Verification rules</h2><Gauge aria-hidden="true" /></div>
      <div className="case-field-grid">
        <label>High-priority threshold<input type="number" min="1" max="100" value={riskRules.highRiskThreshold} onChange={(event) => updateRiskRule('highRiskThreshold', event.target.value)} /></label>
        ...
        <label>Impossible travel window, minutes<input type="number" min="5" max="240" value={riskRules.impossibleTravelWindowMins} onChange={(event) => updateRiskRule('impossibleTravelWindowMins', event.target.value)} /></label>
      </div>
